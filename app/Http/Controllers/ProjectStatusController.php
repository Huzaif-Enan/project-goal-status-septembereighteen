...
class ProjectStatusController extends AccountBaseController
{
    ...
    {
        $project_status = DB::table('projects')->select('projects.pm_id', 'project_status_notifies.project_notify', 'project_status_notifies.extended_days','project_status_notifies.extended_start','deals.project_type','users.name as manager_name', 'projects.client_id', 'clients.name as client_name', 'projects.id', 'projects.project_name', 'projects.project_budget as budget', 'contract_signs.created_at as project_sign','pm_projects.created_at as project_form')
        ...
        ->leftjoin('project_status_notifies', 'project_status_notifies.project_id', 'projects.id')
        ...
        foreach ($project_status as $project) {
            ...
            if($project->extended_start != null && $deliverable_signed_days > $project->extended_start){
            $extended_start= Carbon::parse($project->extended_start);
            $extended_end = Carbon::parse($extended_start)->addHours(24* $project->extended_days);
            if ($deliverable_signed_days > $extended_start && $deliverable_signed_days < $extended_end){
                $diffInHours = $extended_start->diffInHours($deliverable_signed_days);
                $deliverable_signed_days =  Carbon::parse($deliverable_signed_days)->addHours(24 * $project->extended_days);
                $deliverable_signed_days =  Carbon::parse($deliverable_signed_days)->addHours($diffInHours);
        ...
        //-------------------------------------------after 4 days 1st submission for priority to critical sensitive-------------------------------//
           ...
           if ($project->budget >= 501){
               ...
               $priority_submission_client_duration = Carbon::parse($project->project_form)->addHours(96); /////////////////////////////////////////////
                    ...
                    if ($project->extended_start != null && $priority_submission_client_duration > $project->extended_start) {
                        $extended_start = Carbon::parse($project->extended_start);
                        $extended_end = Carbon::parse($extended_start)->addHours(24 * $project->extended_days);
                        if ($priority_submission_client_duration > $project->extended_start && $priority_submission_client_duration < $extended_end) {
                            $diffInHours = $extended_start->diffInHours($priority_submission_client_duration);
                            $priority_submission_client_duration =  Carbon::parse($priority_submission_client_duration)->addHours(24 * $project->extended_days);
                            $priority_submission_client_duration =  Carbon::parse($priority_submission_client_duration)->addHours($diffInHours);
                        } else {
            ...
            //----------------------------------------Milestones Calculation----------------------------------------------------------------------------------//
                ...
                if($project->budget <= 500) {
                    ...
                    if ($project->extended_start != null && $reguler_half_milestone_duration > $project->extended_start) {
                        $extended_start = Carbon::parse($project->extended_start);
                        $extended_end = Carbon::parse($extended_start)->addHours(24 * $project->extended_days);
                        if ($reguler_half_milestone_duration > $project->extended_start && $reguler_half_milestone_duration< $extended_end) {
                            $diffInHours = $extended_start->diffInHours($reguler_half_milestone_duration);
                            $reguler_half_milestone_duration =  Carbon::parse($reguler_half_milestone_duration)->addHours(24 * $project->extended_days);
                            $reguler_half_milestone_duration=  Carbon::parse($reguler_half_milestone_duration)->addHours($diffInHours);
                        } else {
                    ...
                    if ($project->extended_start != null && $one_milestone_release_week_duration > $project->extended_start) {
                        $extended_start = Carbon::parse($project->extended_start);
                        $extended_end = Carbon::parse($extended_start)->addHours(24 * $project->extended_days);
                        if ($one_milestone_release_week_duration > $project->extended_start && $one_milestone_release_week_duration < $extended_end) {
                            $diffInHours = $extended_start->diffInHours($one_milestone_release_week_duration);
                            $one_milestone_release_week_duration =  Carbon::parse($one_milestone_release_week_duration)->addHours(24 * $project->extended_days);
                            $one_milestone_release_week_duration=  Carbon::parse($one_milestone_release_week_duration)->addHours($diffInHours);
                        } else {
                    ...
                    if ($project->extended_start != null && $twelve_days_duration > $project->extended_start) {
                        $extended_start = Carbon::parse($project->extended_start);
                        $extended_end = Carbon::parse($extended_start)->addHours(24 * $project->extended_days);
                        if ($twelve_days_duration > $project->extended_start && $twelve_days_duration < $extended_end) {
                            $diffInHours = $extended_start->diffInHours($twelve_days_duration);
                            $twelve_days_duration=  Carbon::parse($twelve_days_duration)->addHours(24 * $project->extended_days);
                            $twelve_days_duration =  Carbon::parse($twelve_days_duration)->addHours($diffInHours);
                        } else {
     ...
     if($this->user_role_id == 1){
            ...
            $this->view_project_status = ProjectGoalStatus::select('project_goal_statuses.*', 'project_status_notifies.id as notify_id', 'project_status_notifies.project_notify', 'users.name as manager_name', 'projects.project_name', 'projects.project_budget', 'clients.name as client_name', 'pm_projects.created_at as project_start')
            ...
            ->join('p_m_projects as pm_projects', 'pm_projects.project_id', 'projects.id')
            ->leftJoin('project_status_notifies', 'project_status_notifies.project_id', '=', 'project_goal_statuses.project_id')
            ->orderBy('project_goal_statuses.project_id', 'DESC')
            ->groupBy('project_goal_statuses.project_id')
            ->get();
    ...
    public function projectGoalDetails($project_id)
    {
        ...
        if ($this->user_role_id == 1) {
            ...
            ->join('projects', 'projects.id', '=', 'project_goal_statuses.project_id')
            ->join('p_m_projects as pm_projects', 'pm_projects.project_id', 'projects.id')
            ->where('project_goal_statuses.project_id', $project_id)
            ->orderBy('project_goal_statuses.event_date', 'DESC')
            ->get();
        } else {
        ...
        return view('projects.projectgoaldetails', $this->data);
    }
    ...
    public function extendRequestForm($id)
    {

        $this->id = $id;
        //$project_status = ProjectGoalStatus::find($id);
        $this->view_project_status = Project::select('projects.*', 'clients.name as client_name')
        ->join('users as clients',  'clients.id', '=', 'projects.client_id')
        ->where('projects.id', $id)
        ->first();

        return view('projects.extendRequestForm', $this->data);
    }

    public function extendReasonEvent(Request $request)
    {
        
        $project_goal_extend_find = ProjectStatusNotify::select('project_status_notifies.id')
        ->where('project_id', $request->project_id)
        ->first();
       
       
        if($project_goal_extend_find == Null){

            $project_status_notify = new ProjectStatusNotify();
            $project_status_notify->project_id= $request->project_id;
            $project_status_notify->extended_start = Carbon::now();
            $project_status_notify->extended_days = $request->extended_days;
            $project_status_notify->pm_reason = $request->pm_reason;
            $project_status_notify->project_notify=0;
            $project_status_notify->save();

        } else{
            $project_goal_extend = ProjectStatusNotify::find($project_goal_extend_find->id);
            $project_goal_extend->extended_start = Carbon::now();
            $project_goal_extend->extended_days = $request->extended_days;
            $project_goal_extend->pm_reason = $request->pm_reason;
            $project_goal_extend->project_notify = 0;
            $project_goal_extend->save(); 
    ...
    public function reviewRequestExtend($id)
    {
      
        $this->project_status_notify = ProjectStatusNotify::join('projects', 'projects.id', '=', 'project_status_notifies.project_id')
                         ->join('users as clients',  'clients.id', '=', 'projects.client_id')
                         ->join('users as managers',  'managers.id', '=', 'projects.pm_id')
                         ->where('project_status_notifies.id',$id)
                         ->select('projects.*', 'project_status_notifies.*', 'clients.name as client_name','managers.name as manager_name')
                         ->first();

        return view('projects.reviewRequestExtend', $this->data);
    }

    public function extendReviewEvent(Request $request)
    {
        // dd($request);
        ...
        // ->where('project_id', $request->project_id)
        //     ->first();
        $project_status_notify = ProjectStatusNotify::find($request->notify_id);
        if($request->action == "Accept"){
            $project_status_notify->extended_days= $request->extended_days;
            $project_status_notify->admin_comment= $request->admin_comment;
            $project_status_notify->project_notify = 1;
            $project_status_notify->save();
        }else{
            $project_status_notify->extended_days =0;
            $project_status_notify->admin_comment = $request->admin_comment;
            $project_status_notify->project_notify = 1;
            $project_status_notify->save();
